...
with Progress(console=console, transient=True) as progress:
    task = progress.add_task("[cyan] Running ...", total=3)
...
os.system("clear")
output = render(' Moscow  ', 'block', colors=['green', 'red'], align='center')
print(output)
print('\n')
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s [ Moscow • 💥  ] %s/%s ➙ [ [ OK ] ] %s ➙ [ [ CP ] ] %s ..  %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
